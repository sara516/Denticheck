...
GV.initialize_page.centres_page = async function(){
//   displayPlaceholder()


}

onClick(".centre_card",function(){
    navigate_to("centre_page")
})
...
GV.initialize_page.centre_page = async function(){
//   displayPlaceholder()
...
GV.initialize_page.setting = async function(){
  ...
  function displayListTasks(){
    for(let id of Object.keys(GV.tasklists)){
      ...
      <div  class="table_items grid colmn4 padding_top15 text_color1 center ">
          ...
          <div>
            <div class="dropdown">
              <i class="fas fa-ellipsis-v dropbtn dropbtn_tasklist"  style="font-size: 20px;padding: 10px;"></i>
                <div id="myDropdown_tasklist_${tasklist.id}"  class="dropdown-content">
                <div class="action" id="edit_subscription"><i class="fa-solid fa-info light_grey padding5"></i>Modifier</div>
                <div class="action" id="delete_subscription"><i class="fas fa-trash-alt light_grey padding5"></i>Supprimer</div>
                </div>
            </div>
          </div>  
      </div>      `
  ...
  function displaySideListTasks(id, obj, remove){
    var side = {id : "", title_add: "Ajouter Une liste" , title_update: "Modifier La liste",  btn_add: "add_listtasks" , btn_update: "update_listtasks", data_id : id }
    var arr = [
      {data_id : 'title',uniqueClass :'form_listtasks' , class : 'required' , selector : 'input', type : 'text', label : "Nom de la liste", id : '', placeholder :"Titre" },
